...
* 现代操作系统的特征包括__并行__，__虚拟__，__异步__，__共享__。
...
- 请总结你认为操作系统应该具有的特征有什么？并对其特征进行简要阐述。
  * 虚拟性：对硬件进行抽象，不针对某一特定型号
  * 并发性：程序可分时执行，看起来是同时的
  * 异步性：系统调用、中断处理，不会阻塞程序的执行
  * 共享性：多用户、多程序可共享系统资源
  * 持久性：数据可以储存在硬盘等持久化存储设备上


- 为什么现在的操作系统基本上用C语言来实现？为什么没有人用python，java来实现操作系统？

  C语言可以直接操作底层硬件，运行时开销极小、性能高。Python、Java等有虚拟机的语言无法直接操作底层，性能低。
